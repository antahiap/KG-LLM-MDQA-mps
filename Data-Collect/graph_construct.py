...
def tfidf_kw_extract_chunk(d, n_kw, ngram_l, ngram_h):
    ...
    score = X.todense()
    kws_0 = list(set(list(term[(-score).argsort()[:, :n_kw]][0]) + list(titles)))

    #print(kws_0)
    kws_1 = [word.lower() for word in kws_0]
    # Add terms to vocabulary ensuring uniqueness
    kws =[]
    for term in kws_1:
        if term not in kws:
            kws.append(term)
...
def knn_embs(i_d):
    ...
    n_device = set_num_device(idx)
    emb = encoder.encode(chunks, device = n_device )
...
def mdr_embs_construct(dataset, docs, args):
    ...
    # tokenizer = AutoTokenizer.from_pretrained(args.mdr_model_name)
    bert_config = AutoConfig.from_pretrained(args.mhop_model_name)
    bert_config = AutoConfig.from_pretrained(args.mhop_model_name)
...
def set_num_device(idx):

    # Check and set the device
    if torch.cuda.is_available():
        device = torch.device("cuda")
        device_set = 'cuda:{}'.format(idx % torch.cuda.device_count())
        ...
        device_set = 'cpu:{}'.format(idx % torch.cpu.device_count())
    return device_set
...
if __name__ == '__main__':
    ...
    test_docs = json.load(open('./Data-Collect/{}/test_docs.json'.format(args_dataset), 'r'))

    if args_kg == 'TF-IDF':

        test_docs = tfidf_kw_extract(test_docs, args_n_kw, args_min_n, args_max_n, args_n_processes)
        test_docs_graphs = kw_process_graph(test_docs)

        info = 'TF-IDF_{}_{}_{}'.format(args_n_kw, args_min_n, args_max_n)

    elif args_kg == 'KNN':
        if not os.path.exists('./{}/knn_embs.pkl'.format(args_dataset)):
            try:
                mp.set_start_method('spawn')
            except RuntimeError:
                pass
            # mp.set_start_method('spawn')
            knn_embs_construct(args_dataset, test_docs, args_n_processes)

        test_docs_graphs = knn_graph_construct(args_dataset, test_docs, args_k_knn, args_n_processes)
       
        info = 'KNN_{}'.format(args_k_knn)
